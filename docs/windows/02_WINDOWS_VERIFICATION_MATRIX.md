...
| Coordinator protocol v1 | request/event/error contract tests | partial: real TotalSegmentator, fixed DentalSegmentator, fixed Individual Teeth, and fixed ToothSeg normal completion pass on the Windows 10 engineering host; existing typed cancellation remains green; Windows 11 unverified |
...
| WPF representative UI | keyboard, UIA, high contrast, DPI | partial: manual-aligned setup/input/run/result views, four-model comparison copy/images, fixed DentalSegmentator, Individual Teeth, and ToothSeg selection, protocol v1 event mapping, clean-DICOM audit/first-series conversion/series selection, explicit strict-CUDA completion, typed no-clean-series failure, real cancellation, automation names, focusable controls, dynamic system colors, and 96-DPI rendering pass on the Windows 10 engineering host; external UI Automation, actual keyboard traversal, high contrast, other DPI/viewport sizes, native file-dialog interaction, DICOM timeout/cancel UI interaction, secondary-capture rescue, self-contained distribution, clean model distribution, and Windows 11 remain unverified |
...
The fixed ToothSeg path now has Windows 10 engineering evidence on one
RTX 2060 host. This does not close the feature-parity gate: Windows 11,
clean model distribution, repeated-run qualification, and the supported
hardware/VRAM matrix remain unverified.
