...
DEPENDENCY_SET_ID="${TOTALSEGMENTATOR_WRAPPER_MAC_DEPENDENCY_SET_ID:-macos-arm64-py312-torch2.12-totalseg2.14.0-nnunetv2.8.1-pydicom3}"
...
if [[ -z "${LICENSE_SITE_PACKAGES}" ]]; then
  ...
  "${LICENSE_INVENTORY_ENV_PYTHON}" -m pip install -c "${CONSTRAINTS_PATH}" "${WHEEL_PATH}[dicom,mps,dentalseg]" >/dev/null
